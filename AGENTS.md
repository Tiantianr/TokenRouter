...
- 发版前完成 [本地验证门禁](docs/operations/personal_release.md#release_preflight)：涉及数据库、事务、共享测试夹具或 outbox 时，必须跑完整受影响的集成测试包，不能只跑新增用例后等待 CI 发现测试污染；有效验证结果不因打 tag 机械重跑。
- 长时间备份前完成 [容量与恢复预检](docs/operations/personal_release.md#backup_restore_preflight)：按实际文件系统计算 dump、恢复库、WAL/索引及业务增长的峰值空间；用实际 PostgreSQL 镜像和挂载先完成启动、读写与清理冒烟，确认降权后的目录可访问。
- CI、ARM64 构建与通过预检的在线备份恢复应并行推进；应用切换必须等发布成功、备份持久落盘及恢复验证全部通过。有停机或禁止混跑要求的迁移仍遵守专门顺序。
- 维护任务按阶段记录状态、退出码和产物校验和，超时先查状态；完整 dump 在同一升级窗口内校验合格后从恢复阶段续跑，不因后续权限或验收失败重新导出。不得把硬编码旧版本、PID 或目录的一次性脚本当作通用发布工具。
