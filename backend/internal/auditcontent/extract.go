...
func appendResponsesItem(document *Document, value any, current bool) {
	...
	case map[string]any:
		...
		case "additional_tools":
			// Codex首轮input内的工具声明不属于历史调用，仍提取每个工具定义。
			tools, exists := typed["tools"]
			if !exists {
				markIncompleteContent(document)
			} else {
...
func hasRecognizedObjectField(values map[string]any, fields []string) bool {
	for _, field := range fields {
		...
		// 空类型不能证明对象已分类；其它配置字段按存在性识别，false/0/null均可能有效。
...
func markKnownStringOrObjectFields(*Document, any, ...string) {
	// 嵌套配置可能是字符串或对象；缺少适配和无法提取的内容必须可观测。
}

func extractAlphaSearch(document *Document, root map[string]any) {
	// PAT回退把这三个字段序列化为用户提示，必须在选择转发路径前提取同一内容。
...
// 限制不完整原因的数量与长度，避免把客户端可控类型原样写入日志。
...
// 返回结构化片段的统一脱敏表示，便于转换提示的调用方复用内容契约。
...
func markHistoryAttribution(document *Document, role string, source Source) {
	...
	// 当前轮内容分界不等于历史证据：Codex首轮可能发送多条用户上下文。
...
func markUnknownNonEmptyFields(*Document, map[string]any, ...string) {
	// 已识别对象可忽略未知配置字段，但未知内容类型和无法提取的内容必须标记不完整。
}
