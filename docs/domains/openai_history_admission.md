...
| 归属操作因请求取消而中止（`context.Canceled`） | 499 | `request_canceled` |
...
上述本地历史拒绝适用于尚未发生上游失败的请求。Responses、Chat、Messages 和 Responses WebSocket 在上游失败后继续寻找合格账号；严格账号不能接收其它凭据身份的历史，不作为这类请求的兜底。如果后续候选因历史策略耗尽，沿用最后一次上游失败的协议响应与 Ops 归因，不用 `external_history_not_allowed` 的 400 覆盖真实的 429/5xx。客户端仍使用既有安全错误映射，原始上游状态和提取后的错误信息保留在 Ops 中，不直接透传完整上游正文。归属存储故障、请求取消及归属冲突不适用这一优先级，仍独立返回。

归属错误链中的请求取消单独记为逻辑 499、P3、客户端请求分类，不计入平台 SLA 失败。管理端和用户端错误请求显示“请求取消”并支持筛选；该明确分类保留用于排障，不使用旧的 `context canceled` 文本过滤。归属操作超时和真实数据库故障仍为 503，不凭错误提示推断取消发起方，也不跳过归属持久化后继续输出响应 ID。
