...
- [WebSocket 执行隔离](#websocket_execution_isolation)：修改用户线程、抢占、连接复用或账号连接预算时读取。
...
OpenAI OAuth 可选择启用[历史准入](../domains/openai_history_admission.md)：长期 PostgreSQL 归属优先于短期粘性，严格账号不能因等待、候选切换、粘性逃逸或快照陈旧而接收外部历史。账号缺省关闭，影子继承母账号；预占只维持路由，成功完成的上游响应才确认历史归属。分组会话隔离独立生效并先于账号预占检查。
...
<a id="websocket_execution_isolation"></a>
## WebSocket 执行隔离

原生 WS 的抢占租约按分组、API Key 与原始客户端 thread 隔离；没有 thread 时回退原始 session，再回退显式 `prompt_cache_key`。没有这些身份时不注册跨请求抢占。父子线程共享根 session 仍各自持有租约；同一用户线程的新连接可以抢占旧连接，跨账号重试复用既有登记。passthrough 继续使用独立连接，不参加共享池抢占。

WS 连接亲和及池内复用还包含凭据所有者和身份配置快照，包括收敛模式、seed、设备与生效的指定 session；显式启用账号级固定回合时也包含固定 `turn_id`。`full` 模式对外 thread 相等也不能合并不同用户/已知原始线程的连接。标准 Responses 客户端没有任何线程标识时按已认证 Key 隔离，保留其显式 `previous_response_id` 续链及池内复用；连用户都不可识别的内部请求仅与本次重试共享作用域。所有作用域共用账号连接总预算，不得按用户拆池而放大账号并发。新命名空间不读取旧 session 级连接缓存；只有管理员明确保存启用的[账号级回合配置](../interfaces/openai_upstream.md#codex_account_turn_reuse)才自动补入持久状态，普通客户端回带规则保持不变。

入站连接沿用首帧真实线程，后续省略 thread 时继承；显式切换 thread 必须重新建立客户端连接，避免自动续链引入上一线程的历史。窗口序号和 turn 可以正常推进。保存账号身份配置只影响新请求/连接，已建立 WS 保留其账号快照。上述隔离不修改账号调度、严格历史准入或既有用户、请求、attempt、账号的运维归因记录。
