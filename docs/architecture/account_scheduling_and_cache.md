...
WS 连接亲和及池内复用还包含凭据所有者和身份配置快照，包括收敛模式、seed、设备与生效的指定 session。`full` 模式对外 thread 相等也不能合并不同用户/已知原始线程的连接。标准 Responses 客户端没有任何线程标识时按已认证 Key 隔离，保留其显式 `previous_response_id` 续链及池内复用；连用户都不可识别的内部请求仅与本次重试共享作用域。所有作用域共用账号连接总预算，不得按用户拆池而放大账号并发。新命名空间不读取旧 session 级连接缓存，turn-state 不再跨请求自动回填。
