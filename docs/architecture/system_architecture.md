...
应用级 Wire provider set 包含配置、repository、service、payment、securityaudit、middleware、handler 和 server。`Application` 暴露 `*http.Server`、提示词审计运行时与 `Cleanup`；提示词运行时在监听前启动，所有后台服务在关闭基础设施前停止。
