...
最终身份解析先校验完整原始 User-Agent 的 HTTP 字段合法性，拒绝任意位置的 CR/LF 等控制字符，再去除合法边界空白并配对身份、解析版本。canonical 与 override 在校验前保留原值；非法 canonical 的 UA、originator 和版本整体回退，不能借用其版本号拼成另一份身份。合法的客户端名、平台和终端后缀继续保留，不改变版本选择策略，也不随机化 UA 或 TLS 模板。
