...
- [个人仓库发布与 ID3 更新](personal_release.md)：个人 ARM64 发行、本地验证门禁、容量与恢复冒烟、并行阶段、失败续跑、部署及回退。读取时机：提交发版、启用 Actions、准备备份恢复、更新 ID3、排查发布耗时或修改后台在线升级时必读。
...
- [上游传输安全](upstream_transport_security.md)：代理生命周期、连接池隔离、TLS 指纹路由、账号级 Codex UA、目标/重定向和 Header 安全。读取时机：修改代理、HTTP client、OAuth UA 编辑、TLS profile/router、base URL 或直连回退时读取。
...
- [开发、验证与上游同步](development_workflow.md)：生成代码、测试分层、共享夹具与 outbox 隔离、前后端工具链、发布和 fork 同步规则。读取时机：准备开发环境、改 schema/依赖或测试夹具、运行验证、发布或同步上游时读取。
- [边缘与 HTTP 入口安全](edge_security.md)：长连接入口限制、Docker 直接对端与可信代理、真实 IP 验收、流式传输和边缘防护边界。读取时机：修改 HTTP server、反向代理或 Docker 网络、排查客户端 IP、调整请求限制、SSE 或 WebSocket 时读取。
