...
- [可信客户端 IP](#client_ip_resolution)：修改代理信任、Docker 网络、IP ACL 或运行时安全设置时读取。
...
上述回环地址示例只适用于应用实际看到的直接对端也是回环地址的情况。宿主机上的代理即使访问 `127.0.0.1:<映射端口>`，经过 Docker 端口映射后，应用看到的对端也可能是网桥网关。在严格模式下，未配置该实际对端会使访问日志、用量、审计与 IP ACL 全部使用网桥地址；入口代理已经正确写入转发头也不能弥补应用侧信任配置缺失。

修改代理或 Docker 拓扑时，必须核对实际代理链、应用端口绑定、容器网络/网关和观测到的直接对端。将核实后的准确 IP/CIDR 配入 `server.trusted_proxies`，或逗号分隔的 `SERVER_TRUSTED_PROXIES`。例如确认直接对端是 `172.29.0.1` 时可配置 `172.29.0.1/32`，该地址仅为示例，不能跨环境照搬或扩大成所有私网/所有来源。

有 CDN 或 PROXY protocol 中转时，入口代理必须先仅信任明确的上游来源并恢复客户端地址，再覆盖 `X-Real-IP`、`X-Forwarded-For`，清理不使用的客户端 IP 头。应用侧填写的是实际直接对端，不能把公网中转节点误当成容器内直接对端。严格模式已有配置时，不得以开启无条件信任原始转发头的兼容开关来代替修复代理链。

### 修复与验收

先核对进程环境、实际 YAML 与数据库运行时开关；环境变量优先于 YAML，可信代理列表在启动时读取。修改容器环境后必须重新创建应用容器使配置生效，单纯重启旧容器不会更新环境。配置修复应备份原文件、校验解析后的差异仅包含预期项，并核对应用镜像与依赖容器身份。

公网入口验收必须包含一条普通请求和一条携带伪造转发 IP 头的请求。用唯一探针标识或路径关联入口代理与应用访问日志，确认两者记录同一实际客户端出口地址，伪造值未生效；随后检查新产生的用量等持久记录。容器内 `/health` 的回环/网桥地址不能代替此项验收，也不能只看代理日志就判定应用已正确解析。

修复只影响后续记录。已有记录若仅保存了网桥地址，不能据此推断真实客户端 IP；历史回填必须另有可信且能逐条关联的来源，禁止批量改成当前用户或某次探针的 IP。
