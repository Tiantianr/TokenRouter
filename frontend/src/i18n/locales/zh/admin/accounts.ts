...
// Accounts Management
    accounts: {
      ...
      openai: {
        ...
        accountUserAgentPlaceholder: '留空：不设置账号级覆盖',
        accountUserAgentHint: '清空会移除账号 UA，恢复现有选择规则，并非强制使用全局 UA。TLS 路由规则和 ForceCodexCLI 仍可能覆盖此值。',
        ...
        sessionOverrideEnabled: '启用指定会话 ID',
        sessionOverrideID: '候选会话 ID',
        sessionOverrideCurrent: '已保存配置对应的 ID',
        sessionOverrideTestHint: '同一账号的文字测试复用固定测试会话，可连续测试同一 ID 观察稳定性。单次成功不代表真实续聊或多人并发下持续可用。',
        sessionOverrideReconnectHint: '保存影响新请求。已有 WebSocket 会话保留原身份，请在当前回复结束后重连客户端再验证。',
        sessionOverrideTestIdentity: '本次测试 session_id={session} · thread_id={thread}',
        sessionOverrideRandom: '随机生成会话 ID',
        testTurnStateEnabled: '账号级固定 turn_id 并自动更新回合状态',
        testTurnID: '固定 turn_id',
        testTurnIDRandom: '随机生成 turn_id',
        testTurnStateCaptured: '已有上游回合状态；草稿需点击保存后生效',
        testTurnStateWaiting: '请先测试，等待上游成功响应返回回合状态',
        testTurnStateUpdated: '账号状态最后保存 / 自动更新：{time}',
        testTurnStateHint: '测试时临时捕获 x-codex-turn-state，确认回复正常后点击下方保存，会同时保存会话和回合配置。正式 Responses 请求持续复用固定 turn_id 与当前状态；完整成功响应返回新的非空状态后自动更新，失败或中断保留旧值。随机 turn_id 或更换会话后需重新测试。',
        sessionOverrideDraft: '测试草稿 · 未保存',
        sessionOverrideSaved: '已保存配置',
        sessionOverrideLoadFailed: '会话配置加载失败',
        sessionOverrideSaveFailed: '会话配置保存失败；账号身份或回合状态可能已更新，请重新加载并测试后保存',
