export default {
  promptAudit: {
    sessions: {
      previous: '上一页',
      next: '下一页',
      sessionId: '会话 ID', manage: '管理该会话事件',
      title: '会话证据', description: '按用户和会话查看检测记录。进入会话后选择“查看证据”，可查看检测详情并下载完整上下文。通过请求的普通证据保留七天，指定用户的通过证据及风险证据长期保留。',
      back: '返回会话列表', counts: '事件 / 风险 / 可用证据', lastSeen: '最近活动', open: '查看会话', evidence: '完整上下文',
      available: '可下载', unavailable: '未留存或已过期', view: '查看证据', empty: '暂无会话或检测记录', loadFailed: '加载会话证据失败，请重试'
    },
    title: '提示词审计',
    ...
    policy: {
      ...
      blockingExemptUsers: '免阻塞用户', blockingExemptUsersHint: '这些用户仍会完整执行提示词审计并生成事件；本引擎命中只记录，不阻止请求。原有内容审核规则独立生效，深审状态和内容提取异常仍按安全策略处理。', blockingExemptUserCount: '已选择 {count} 名免阻塞用户，最多 100 名',
    ...
    cleanup: {
      ...
      userScope: '用户范围', allUsers: '全部用户', selectedUser: '指定用户', userRequired: '请选择一个用户。', matched: '匹配 {count} 条正常记录', contexts: '完整聊天份数', estimatedBytes: '预计回收证据数据', estimateHint: '这是证据内容的存储估算；内置逻辑备份不包含正文表，清理也不保证数据库文件立即返还磁盘。', previewRequired: '请先获取服务端预览。', confirm: '确认清理正常记录',
    },
