...
// Accounts
    accounts: {
      ...
      openai: {
        ...
        sessionOverrideEnabled: 'Use a specified session ID',
        sessionOverrideID: 'Candidate session ID',
        sessionOverrideCurrent: 'ID from saved configuration',
        sessionOverrideTestHint: 'Text tests for the same account reuse a stable test client session. Repeat tests with the same ID to observe stability. One success does not establish continued availability with real conversation history or concurrent users.',
        sessionOverrideReconnectHint: 'Saving affects new requests. Existing WebSocket sessions keep their identity; reconnect the client after the current response finishes to verify the change.',
        sessionOverrideTestIdentity: 'Test session_id={session} · thread_id={thread}',
        sessionOverrideRandom: 'Generate a random session ID',
        testTurnStateEnabled: 'Fix account turn ID and automatically update turn state',
        testTurnID: 'Fixed turn ID',
        testTurnIDRandom: 'Generate a random turn ID',
        testTurnStateCaptured: 'Upstream state available; save to apply draft changes',
        testTurnStateWaiting: 'Run a test and wait for a successful response with turn state',
        testTurnStateUpdated: 'Account state last saved / automatically updated: {time}',
        testTurnStateHint: 'Tests capture x-codex-turn-state temporarily. Once satisfied with the response, Save applies both session and turn settings. Responses requests reuse the fixed turn ID and current state, automatically saving a new nonempty state after a complete successful response. Failures and interruptions retain the previous state. Test again after changing the session or randomizing the turn ID.',
        sessionOverrideDraft: 'Test draft · Not saved',
        sessionOverrideSaved: 'Saved configuration',
        sessionOverrideLoadFailed: 'Failed to load session configuration',
        sessionOverrideSaveFailed: 'Failed to save session configuration. The account identity or turn state may have changed; reload and test again before saving.',
