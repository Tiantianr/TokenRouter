  ...
  <BaseDialog :show="show" :title="t('admin.promptAudit.events.filterDeleteDialogTitle')" width="wide" @close="$emit('close')">
    <div class="space-y-5 text-sm">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        <p v-if="local.session_id" class="text-sm sm:col-span-2">{{ t('admin.promptAudit.sessions.sessionId') }}: #{{ local.session_id }}</p>
        <label class="text-xs text-gray-600 dark:text-dark-200">
          <span>{{ t('admin.promptAudit.events.decision') }}</span>
          <Select v-model="local.decision" class="mt-1 w-full" :aria-label="t('admin.promptAudit.events.decision')" data-test="delete-decision" @change="criteriaChanged" :options="[{ value: '', label: t('common.all') }, { value: 'pass', label: t('admin.promptAudit.decisions.pass') }, { value: 'flag', label: t('admin.promptAudit.decisions.flag') }, { value: 'critical', label: t('admin.promptAudit.decisions.critical') }]" />
        </label>
        <label class="text-xs text-gray-600 dark:text-dark-200">
          <span>{{ t('admin.promptAudit.events.risk') }}</span>
          <Select v-model="local.risk_level" class="mt-1 w-full" :aria-label="t('admin.promptAudit.events.risk')" data-test="delete-risk" @change="criteriaChanged" :options="[{ value: '', label: t('common.all') }, { value: 'low', label: t('admin.promptAudit.riskLevels.low') }, { value: 'medium', label: t('admin.promptAudit.riskLevels.medium') }, { value: 'high', label: t('admin.promptAudit.riskLevels.high') }, { value: 'critical', label: t('admin.promptAudit.riskLevels.critical') }]" />
        </label>
      </div>
