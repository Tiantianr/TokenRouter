...
describe('Prompt Audit integration surface', () => {
  ...
  it('keeps both security pages visible under the current risk-control feature gate', () => {
    const sidebar = read('../../../components/layout/AppSidebar.vue')
    const group = sidebar.slice(sidebar.indexOf("path: '/admin/risk-control'"), sidebar.indexOf("path: '/admin/redeem'"))
    expect(group).toContain('risk_control_enabled === true')
    expect(group).not.toContain('hideInSimpleMode')
